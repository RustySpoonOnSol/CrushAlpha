...
export default async function handler(req, res) {
  try {
    const mint = req.query.mint || process.env.NEXT_PUBLIC_CRUSH_MINT || "";
    ...
    const rpc = async (body) =>
      (await fetch(RPC, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ jsonrpc: "2.0", id: 1, ...body }) })).json();

    const largest = await rpc({
      method: "getTokenLargestAccounts",
      params: [mint, { commitment: "confirmed" }],
    });
    if (largest?.error) return res.status(502).json({ error: largest.error?.message });

    const addrs = (largest?.result?.value || []).slice(0, limit).map((x) => x.address);
    const infos = addrs.length
      ? await rpc({ method: "getMultipleAccounts", params: [addrs, { encoding: "jsonParsed" }] })
      : { result: { value: [] } };
