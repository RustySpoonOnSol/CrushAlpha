...
// Prefer your own overrides first (lets us bypass integration-managed envs)
...
export default async function handler(req, res) {
  try {
    ...
    if (
      !(process.env.CRUSH_KV_URL || process.env.KV_REST_API_URL || process.env.UPSTASH_REDIS_REST_URL)
    ) return res.status(500).json({ error: "env_missing_KV_URL" });
    if (
      !(process.env.CRUSH_KV_TOKEN || process.env.KV_REST_API_TOKEN || process.env.UPSTASH_REDIS_REST_TOKEN)
    ) return res.status(500).json({ error: "env_missing_KV_TOKEN" });
    ...
    // Dynamic import keeps build robust if lib changes
    let payments;
    try {
    ...
    // Reference: 32 random bytes → base58
    ...
    // Anti-spoof: bind ref → item for 15 minutes
    try {
