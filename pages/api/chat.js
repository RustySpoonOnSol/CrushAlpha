...
export default async function handler(req, res) {
  try {
    ...
    if (message.length > MAX_INPUT_LEN) return res.status(413).json({ error: 'Message too long' });
    ...
    // ⏱️ Per-wallet cooldown
    const keyId = `w:${wallet}`;
    const now = Date.now();
    const last = lastSendMap.get(keyId) || 0;
    if (now - last < SS_COOLDOWN_MS) {
      const left = Math.ceil((SS_COOLDOWN_MS - (now - last)) / 1000);
...
function sanitizeHistory(arr) {
  ...
  return arr.slice(-16).map((m) => {
    const role = m?.role === 'assistant' || m?.role === 'system' ? m.role : 'user';
    const content = typeof m?.content === 'string' ? m.content.slice(0, MAX_INPUT_LEN) : '';
    return { role, content };
  }).filter(m => m.content);
}
function getIP(req){ return (req.headers['x-forwarded-for']?.toString().split(',')[0].trim()) || req.socket?.remoteAddress || 'unknown'; }
function allow(ip){
  const now = Date.now(); const e = rlMap.get(ip) || { count: 0, ts: now };
  if (now - e.ts > RL_WINDOW_MS) { rlMap.set(ip, { count: 1, ts: now }); return true; }
  if (e.count >= RL_MAX_HITS) return false; e.count += 1; rlMap.set(ip, e); return true;
}
function safeParse(s){ try { return JSON.parse(s); } catch { return null; } }
function getQuery(req, key){ const u = new URL(req.url, 'http://localhost'); return u.searchParams.get(key); }
function parseBool(v, fb=false){ if (v==null) return fb; const s=String(v).toLowerCase(); return s==='1'||s==='true'||s==='yes'||s==='y'; }

/* ===== Solana balance (server) ===== */
async function rpcNode(method, params) {
...
async function getCrushBalance(owner, mint) {
  const tokAccs = await rpcNode('getTokenAccountsByOwner', [ owner, { mint }, { encoding: 'jsonParsed' } ]);
